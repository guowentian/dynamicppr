WORK_DIR=../cpu
LOG_DIR=log
DATA_DIR=/data1/wentian/data #specify your own directory storing the data sets
WINDOW_RATIO=0.1
THREAD_NUM=40
...
#can reduce the workload by using fewer source vertices
SOURCES_START=3
SOURCES_END=4
#SOURCES_START=0
#SOURCES_END=10
    ...
    for ((batchidx=0; batchidx<${#BATCH_SIZES[@]}; batchidx++)); do
        ...
        for SOURCE_FEATURE in ${SOURCE_FEATURES[@]}; do
            ...
            for ((sidx=${SOURCES_START}; sidx<${SOURCES_END}; sidx++)); do
                ...
                COMMAND="${WORK_DIR}/pagerank -d ${DATA_DIR}/${DATA_SET} -a ${APP_TYPE} -i ${DIRECTION} -y 1 -n 1 -c ${BATCH_SIZE} -l ${TOTAL_EDGE_NUM} -s ${sid} -t ${THREAD_NUM} > ${LOG_DIR}/${OUTPUT_FILE}"
                echo "command=${COMMAND}"
                
                ${WORK_DIR}/pagerank -d ${DATA_DIR}/${DATA_SET} -a ${APP_TYPE} -i ${DIRECTION} -y 1 -n 1 -c ${BATCH_SIZE} -l ${TOTAL_EDGE_NUM} -s ${sid} -t ${THREAD_NUM} > ${LOG_DIR}/${OUTPUT_FILE}                                                                                                                                                                  
            done
        done
    done
done
    ...
    do
        for SOURCE_FEATURE in ${SOURCE_FEATURES[@]}; do
            ...
            for ((sidx=${SOURCES_START}; sidx<${SOURCES_END}; sidx++)); do
                ...
                COMMAND="${WORK_DIR}/pagerank -d ${DATA_DIR}/${DATA_SET} -a ${APP_TYPE} -i ${DIRECTION} -y 1 -n 0 -r ${BATCH_RATIO} -b ${BATCH_COUNT} -s ${sid} -t ${THREAD_NUM} -o ${variant} > ${LOG_DIR}/${OUTPUT_FILE}"
                echo "command=${COMMAND}"
                ${WORK_DIR}/pagerank -d ${DATA_DIR}/${DATA_SET} -a ${APP_TYPE} -i ${DIRECTION} -y 1 -n 0 -r ${BATCH_RATIO} -b ${BATCH_COUNT} -s ${sid} -t ${THREAD_NUM} -o ${variant} > ${LOG_DIR}/${OUTPUT_FILE}
    ...
    do
        ...
        for SOURCE_FEATURE in ${SOURCE_FEATURES[@]}; do
            ...
            for ((sidx=${SOURCES_START}; sidx<${SOURCES_END}; sidx++)); do
                ...
                COMMAND="${WORK_DIR}/pagerank -d ${DATA_DIR}/${DATA_SET} -a ${APP_TYPE} -i ${DIRECTION} -y 1 -n 0 -r ${BATCH_RATIO} -b ${BATCH_COUNT} -s ${sid} -t ${THREAD_NUM} -e ${error} > ${LOG_DIR}/${OUTPUT_FILE}"
                echo "command=${COMMAND}"
                ${WORK_DIR}/pagerank -d ${DATA_DIR}/${DATA_SET} -a ${APP_TYPE} -i ${DIRECTION} -y 1 -n 0 -r ${BATCH_RATIO} -b ${BATCH_COUNT} -s ${sid} -t ${THREAD_NUM} -e ${error} > ${LOG_DIR}/${OUTPUT_FILE}
...
for ((di=0; di<${#DATA_SETS[@]}; di++)); do
    ...
    for SOURCE_FEATURE in ${SOURCE_FEATURES[@]}; do
        ...
        for ((sidx=${SOURCES_START}; sidx<${SOURCES_END}; sidx++)); do
            ...
            COMMAND="${WORK_DIR}/pagerank -d ${DATA_DIR}/${DATA_SET} -a ${APP_TYPE} -i ${DIRECTION} -y 1 -n 0 -r ${BATCH_RATIO} -b ${BATCH_COUNT} -s ${sid} -t ${THREAD_NUM} > ${LOG_DIR}/${OUTPUT_FILE}"
            echo "command=${COMMAND}"
            ${WORK_DIR}/pagerank -d ${DATA_DIR}/${DATA_SET} -a ${APP_TYPE} -i ${DIRECTION} -y 1 -n 0 -r ${BATCH_RATIO} -b ${BATCH_COUNT} -s ${sid} -t ${THREAD_NUM} > ${LOG_DIR}/${OUTPUT_FILE}
...
scalability (){
OUTPUT_FILE_PREFIX=scalability
THREAD_NUMS=(1 8 16 24 32 40)
BATCH_RATIO=0.01
BATCH_COUNT=100
for ((di=0; di<${#DATA_SETS[@]}; di++)); do
    DATA_SET=${DATA_SETS[$di]}
    ...
    do
        for SOURCE_FEATURE in ${SOURCE_FEATURES[@]}; do
            ...
            for ((sidx=${SOURCES_START}; sidx<${SOURCES_END}; sidx++)); do
                sid=${SOURCES[$sidx]}
                OUTPUT_FILE="${OUTPUT_FILE_PREFIX}_${THREAD_NUM}_${DATA_SET}_${sid}.txt"
